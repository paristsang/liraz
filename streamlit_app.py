...
def fetch_static_map_png(lat: float, lon: float, zoom: int = 13, w: int = 900, h: int = 420) -> bytes | None:
    """
    Best-effort static map image. If blocked, return None.
    """
    url = (
        "https://staticmap.openstreetmap.de/staticmap.php"
        f"?center={lat},{lon}&zoom={zoom}&size={w}x{h}&maptype=mapnik"
...
def fallback_map_png(lat: float, lon: float, w: int = 900, h: int = 420) -> bytes:
    """
...
def build_pdf(
    ...
    prob_txt = "N/A" if prob_landslide is None else f"{prob_landslide*100:.2f}%"
    story.append(Paragraph(f"<b>Prediction:</b> {pred_label}", styles["Normal"]))
    story.append(Paragraph(f"<b>P(landslide):</b> {prob_txt}", styles["Normal"]))
    story.append(Spacer(1, 10))

    # Map image
    story.append(Paragraph("Map", styles["H2"]))
    map_bytes = fetch_static_map_png(lat, lon) or fallback_map_png(lat, lon)
...
if hasattr(model, "predict_proba") and hasattr(model, "classes_"):
    ...
    pred_label = label_from_class(pred_class, positive_class)
else:
    # regression or no proba: show raw prediction
...
with col_left:
    ...
    color = "red" if str(pred_label).lower() == "landslide" else "green"
    folium.CircleMarker(
        location=[lat, lon],
        radius=8,
        color="#111827",
...
with col_right:
    st.subheader("Result")
    if str(pred_label).lower() in ["landslide", "non-landslide"]:
        if str(pred_label).lower() == "landslide":
    ...
    if prob_landslide is not None:
        st.metric("P(landslide)", f"{prob_landslide*100:.2f}%")
    else:
        st.caption("Probability not available (model has no predict_proba).")
